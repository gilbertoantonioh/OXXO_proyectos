...
BEGIN
   ...
   COMMIT;  
    ...
    dbms_output.put_line('NOTA: SE DEBE EJECUTAR LA COMPILACION DEL FLEXFIELD MTL_SYSTEM_ITEMS DESDE EL SISTEMA.');
   
   BEGIN 
      dbms_output.put_line('Inicia ejecucion de compilacion del flexfield MTL_SYSTEM_ITEMS.');
      fnd_global.apps_initialize(0, 20419, 0);
      ln_request_id := fnd_request.submit_request( application      => 'FND' 
                                                 , program          => 'FDFVGN'
                                                 , argument1        => '3'
                                                 , argument3        => '401'
                                                 , argument2        => 'MTL_SYSTEM_ITEMS'
                                                  );    
      
      COMMIT;
      dbms_output.put_line('Finaliza ejecucion de compilacion del flexfield MTL_SYSTEM_ITEMS. Request_id: '||ln_request_id);   
   EXCEPTION 
      WHEN OTHERS THEN 
	     ROLLBACK; 
		 dbms_output.put_line('Error al ejecutar la compilacion del flexfield MTL_SYSTEM_ITEMS. Favor de hacerlo desde el sistema. Error: '||SQLERRM);   
   END;
EXCEPTION
   WHEN OTHERS THEN
      ROLLBACK;
