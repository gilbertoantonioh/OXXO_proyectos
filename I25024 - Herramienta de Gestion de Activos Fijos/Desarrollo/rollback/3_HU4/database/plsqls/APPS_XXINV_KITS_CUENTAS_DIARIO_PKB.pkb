...
-----------------------------------
   ...
   pre_xxinv_mat_trx_temp_prc (pd_date => p_fecha, pv_retcode => retcode);
   ...
   reagrupa_info_prc (pd_creation_date => p_fecha);
   ----
...
END CREATE_CODE_COMBINATION;
   ...
   *
   ********************************************************************************************/
   PROCEDURE pre_xxinv_mat_trx_temp_prc (pd_date IN DATE, pv_retcode IN OUT VARCHAR2) IS
      CURSOR cur_preinsert_mat_trx_tmp (cp_date IN DATE)
      IS 
         SELECT *
         FROM xxinv_material_trx_temp
         WHERE 1=1
         AND TRUNC(mmt_creation_date) = cp_date
   ...
   BEGIN
      BEGIN
         INSERT INTO xxinv_pre_material_trx_temp
            ...
            FROM    xxinv_material_trx_temp
            WHERE   1 = 1
            AND     TRUNC(mmt_creation_date) = pd_date
   ...
   ********************************************************************************************/
   ...
   IS
      ...
      CURSOR cur_pre_mat_trx (p_date IN DATE) IS
         SELECT *
         FROM xxinv_pre_material_trx_temp
         WHERE TRUNC(mmt_creation_date) = p_date
         FOR UPDATE;

      CURSOR cur_info_escaner (p_viaje IN VARCHAR2, p_articulo IN VARCHAR2) IS
   ...
   BEGIN
      ...
      FOR regPMAdd IN cur_pre_mat_trx (pd_creation_date) LOOP
         ...
         FOR InfScan IN cur_info_escaner (regPMAdd.mmt_viaje, regPMAdd.msi_segment1_no_articulo) LOOP
   ...
   ********************************************************************************************/
   PROCEDURE reagrupa_info_prc (
               pd_creation_date     IN DATE) IS
      ln_dias_depuracion NUMBER;            
   BEGIN
