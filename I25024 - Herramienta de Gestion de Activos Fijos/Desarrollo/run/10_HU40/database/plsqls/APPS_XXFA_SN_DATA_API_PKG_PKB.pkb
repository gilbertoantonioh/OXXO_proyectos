...
AS 
   ...
   * Gilberto Hernandez (Hexaware)   10/Abr/2026  CHG0146509      El programa load_trips_from_wsh_prc se cicla en escenarios donde habia nuevas lineas de wsh vs sn.
   * Gilberto Hernandez (Hexaware)   9/Abr/2026   CHG0145709      Para insertar otras recepciones (Directos) se debe tener la po/release validada mediante el programa XXFA - SN Validacion de Orden de Compra Cargos Directos.  
   ********************************************************************************************/
   ...
   * Gilberto Hernandez (Hexaware)   9/Abr/2026   CHG0145709      Se debe tener la po/release validada mediante el programa XXFA - SN Validacion de Orden de Compra Cargos Directos.  
   ********************************************************************************************/
   ...
   IS 
	  ...
	  lv_po_validated_flag      po_headers_all.global_attribute20%TYPE :=  NULL; 
   ...
   BEGIN 
	  ...
	  -- Inicia CHG0145709
	  -- Se debe tener la po/release validada mediante el programa XXFA - SN Validacion de Orden de Compra Cargos Directos.  
      BEGIN 
         SELECT DECODE(poh.type_lookup_code, 'STANDARD', poh.global_attribute20
                                           , 'BLANKET', pra.global_attribute20
										   , NULL 
                      ) po_validated_flag   
		 INTO   lv_po_validated_flag			  
	  ...
	  EXCEPTION
	     WHEN OTHERS THEN 
            gv_retcode := '2';
            write_errbuf_prc('Error al buscar la transaccion de recepcion con ID '||p_rcv_transaction_id||', relacionada a una linea de recepcion/compra: '||SQLERRM||'. (load_det_others_from_rcv_prc)');
            RAISE handled_exception;		    
	  END;
	  -- Finaliza CHG0145709	  
      ...
      EXCEPTION  
         WHEN OTHERS THEN    
            gv_retcode := '2';
            write_errbuf_prc('Error al buscar la transaccion de recepcion con ID '||p_rcv_transaction_id||', relacionada a una linea de recepcion: '||SQLERRM||'. (load_det_others_from_rcv_prc)');
            RAISE handled_exception;
      END;
	  ...
	  -- Inicia CHG0145709
	  -- Se debe tener la po/release validada mediante el programa XXFA - SN Validacion de Orden de Compra Cargos Directos.  	  
	  IF NVL(lv_po_validated_flag, 'N') != 'Y' 
      THEN 
         gv_retcode := '1'; 
         write_errbuf_prc('La orden de compra o despacho de la transaccion de recepcion con ID '||p_rcv_transaction_id||' no ha sido validada mediante la ejecucion del programa concurrente XXFA - SN Validacion de Orden de Compra Cargos Directos.');    
	  END IF; 
	  -- Finaliza CHG0145709	  
