SET DEFINE OFF;
PROMPT PACKAGE BODY APPS.XXINV_KITS_CUENTAS_DIARIO
create or replace PACKAGE BODY           apps.XXINV_KITS_CUENTAS_DIARIO IS
--=================================================================================================
...
-- Gilberto Hernandez     7-Octubre-2025  CHG0116809   Actualización de atributos de activos en la etapa previa a la salida de almacén
-- Gilberto Hernandez     4-Abril-2026    CHG0145297   Actualización de atributos de activos al actualizar las tablas de escaneo para Service Now. 
--=================================================================================================
...
FND_FILE.PUT_LINE(FND_FILE.LOG,'Ingresa XXINV_KITS_CUENTAS_DIARIO' );
   ...
   (REQH_SEGMENT1_NO_REQUISICION,  REQH_REQUISITION_HEADER_ID,
        ...
        RL_ATTRIBUTE1_REQ_LINE_ID,AF_CCID, MSI_PURCHASING_TAX_CODE , MTA_GL_BATCH_ID,MMT_ATTRIBUTE15, MMT_MAT_TRX_TMP_ID)
...
--        reqs.attribute2 REQL_ATTRIBUTE2_CRSUP,
        ...
        MMT.DISTRIBUTION_ACCOUNT_ID AF_CCID,MSI.PURCHASING_TAX_CODE ,MTA.GL_BATCH_ID,MMT.ATTRIBUTE15, XXINV_REF_LINES_S.NEXTVAL
...
--------------------
   ...
   --AND XXINV_KITS_CUENTAS_DIARIO.BUSCA_REQUIS_CR_SUP(rl.attribute1) = '10VCZ'
   --AND ROWNUM < 130
   ;--36-change(Deferred Cost of Goods Sold)WMS 23-03-17
   --Fin ChO ***51967670***
...
-----------------------------------
   ...
   -- 03092025 MVL
   -- Se agrega procedimiento para insertar informacion de forma granular por articulo,
   -- ademas de obtener los datos de precio de compra
   fnd_file.put_line (fnd_file.LOG,'Procedure main - Haciendo llamado a pre_xxinv_mat_trx_temp_prc. ' || SYSTIMESTAMP );
   pre_xxinv_mat_trx_temp_prc (pd_date => p_fecha
                             , pv_retcode => retcode
							 , pv_errors => errbuf  -- HERNAGI: Se agrega parametro de salida de errores
							  );

   fnd_file.put_line (fnd_file.LOG,'Procedure main - Haciendo llamado al proceso de llenado de precios: procesa_info_articulo_prc. ' || SYSTIMESTAMP );
   procesa_info_articulo_prc (pd_creation_date => p_fecha
                            , pv_retcode => retcode -- HERNAGI: Se agrega parametro de salida de errores
						    , pv_errors => errbuf  -- HERNAGI: Se agrega parametro de salida de errores
                              );

   fnd_file.put_line (fnd_file.LOG,'Procedure main - Se llama al procedimiento que reagrupa la informacion: reagrupa_info_prc. ' || SYSTIMESTAMP );
   reagrupa_info_prc (pv_retcode => retcode -- HERNAGI: Se agrega parametro de salida de errores
				    , pv_errors => errbuf  -- HERNAGI: Se agrega parametro de salida de errores
                      );
   ----
         ...
         IF W_KITS_SALDO.ITEM_NO = W_MATERIAL_TRX.MSI_SEGMENT1_NO_ARTICULO  THEN
            ...
            ELSE
               IF W_CANTIDAD_PENDIENTE > 0 THEN
                  ...
                  INSERT INTO XXINV_MATERIAL_TRX_TEMP  --xxfc.XXINV_MATERIAL_TRX_TEMP --23/Abr/2018 EMMR Consiss  Upgrade R12.2.4
                         ( REQH_SEGMENT1_NO_REQUISICION
                          ,REQH_REQUISITION_HEADER_ID
                          ,REQL_ATTRIBUTE2_CRSUP
                          ,REQL_ATTRIBUTE3_CR
                          ,MSI_SEGMENT1_NO_ARTICULO
                          ,MSI_DESCRIPCION
                          ,MSI_ATTRIBUTE1_USO
                          ,MSI_ATTRIBUTE2_MAE_ACTIVO
                          ,MSI_ATTRIBUTE3_CUENTA
                          ,MSI_ATTRIBUTE7_MAE_TIPO
                          ,MSI_ATTRIBUTE8_MAE_YN
                          ,MSI_ATTRIBUTE4_PARENT_MAND
                          ,MSI_ATTRIBUTE6_PARENT_HIJOS
                          ,MSI_PURCHASING_TAX_CODE
                          ,MMT_TRANSACTION_ID
                          ,MMT_CREATION_DATE
                          ,MMT_CREATED_BY
                          ,MMT_INVENTORY_ITEM_ID
                          ,MMT_TRANSACTION_QUANTITY
                          ,MMT_TRANSACTION_DATE
                          ,MMT_DISTRIBUTION_ACCOUNT_ID
                          ,MMT_ACTUAL_COST
                          ,MMT_SOURCE_CODE
                          ,MMT_ATTRIBUTE1
                          ,MMT_ATTRIBUTE2
                          ...
                          ,MMT_TRANSACTION_ACTION_ID
                          ,MMT_TRANSACTION_SOURCE_TYPE_ID
                          ,MMT_ORGANIZATION_ID
                          ,MTA_GL_BATCH_ID
                          ,RL_LINE_ID
                          ,RH_HEADER_ID
                          ,RL_ATTRIBUTE1_REQ_LINE_ID
                          ,AF_ORACLE_EF
                          ,AF_ORACLE_CIA
                          ,AF_CCID
                          ,AF_INTERFACE_TYPE
                          ,AF_TIPO_USO_CONVERTIDO
                          ,AF_STATUS_KITS
                          ,AF_STATUS_CUENTAS
                          ,MMT_ACTUAL_COST_ALM
                         )
                  VALUES ( W_MATERIAL_TRX.REQH_SEGMENT1_NO_REQUISICION
                          ...
                          ,W_MATERIAL_TRX.MMT_ACTUAL_COST_ALM
                          );
...
END CREATE_CODE_COMBINATION;
   ...
   ********************************************************************************************/
   PROCEDURE pre_xxinv_mat_trx_temp_prc (pd_date              IN DATE   DEFAULT NULL --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
      ...
      CURSOR cur_preinsert_mat_trx_tmp (p_date                IN DATE
                                      , pn_ooh_order_number   IN NUMBER --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
                                      , pn_ool_line_id        IN NUMBER --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
										)
      IS
         SELECT *
         FROM xxinv_material_trx_temp
         WHERE 1=1
		 AND  (  ( TRUNC(mmt_creation_date) = p_date AND pn_ooh_order_number IS NULL )  --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
		    OR   ( rh_header_id = pn_ooh_order_number AND  rl_line_id = pn_ool_line_id AND pn_ooh_order_number IS NOT NULL ) --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
			  )
		 AND ABS(mmt_transaction_quantity) > 1;

         ln_Error        NUMBER:= 0;
		 lv_Error        VARCHAR2(4000);  --HERNAGI. Agregar paramaetro de salida de error.

   BEGIN
      BEGIN
         INSERT INTO xxinv_pre_material_trx_temp
                  (  reqh_segment1_no_requisicion
                   , reqh_requisition_header_id
                   , reql_attribute2_crsup
                   , reql_attribute3_cr
            ...
            SELECT  reqh_segment1_no_requisicion
                  , reqh_requisition_header_id
                  , reql_attribute2_crsup
                  , reql_attribute3_cr
                  , msi_segment1_no_articulo
                  ...
                  , af_status_kits
                  , af_status_cuentas
                  , mmt_mat_trx_tmp_id -- Id de referencia
                  , get_viaje_fnc(rh_header_id, rl_line_id)
            FROM    xxinv_material_trx_temp
            WHERE   1 = 1
		    AND  (  ( TRUNC(mmt_creation_date) = pd_date AND pn_ooh_order_number IS NULL )  --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
		       OR   ( rh_header_id = pn_ooh_order_number AND  rl_line_id = pn_ool_line_id AND pn_ooh_order_number IS NOT NULL ) --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
			     )
			AND     ABS(NVL(mmt_transaction_quantity,0)) = 1
			;
      END;

      FOR i IN cur_preinsert_mat_trx_tmp (pd_date
	                                    , pn_ooh_order_number --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
										, pn_ool_line_id --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
										 )
      LOOP
         FOR qty IN 1 .. ABS(i.mmt_transaction_quantity)
         LOOP
            BEGIN
               INSERT INTO xxinv_pre_material_trx_temp
                  (  reqh_segment1_no_requisicion
               ...
               VALUES
                  (  i.reqh_segment1_no_requisicion
                   , i.reqh_requisition_header_id
                   , i.reql_attribute2_crsup
                   , i.reql_attribute3_cr
                   , i.msi_segment1_no_articulo
                   , i.msi_descripcion
                   , i.msi_attribute1_uso
                   , i.msi_attribute2_mae_activo
                   , i.msi_attribute3_cuenta
                   , i.msi_attribute7_mae_tipo
                   , i.msi_attribute8_mae_yn
                   , i.msi_attribute4_parent_mand
                   , i.msi_attribute6_parent_hijos
                   , i.msi_purchasing_tax_code
                   , i.mmt_transaction_id
                   , i.mmt_creation_date
                   , i.mmt_created_by
                   , i.mmt_inventory_item_id
                   , -1 --mmt_transaction_quantity -- REVISAR
                   , i.mmt_transaction_date
                   , i.mmt_distribution_account_id
                   , i.mmt_actual_cost
                   , i.mmt_source_code
                   , i.mmt_attribute1
                   , i.mmt_attribute2
                   ...
                   , i.mmt_attribute14
                   , i.mmt_attribute15
                   , i.mmt_transaction_type_id
                   , i.mmt_transaction_action_id
                   , i.mmt_transaction_source_type_id
                   , i.mmt_organization_id
                   , i.mta_gl_batch_id
                   , i.rl_line_id
                   , i.rh_header_id
                   , i.rl_attribute1_req_line_id
                   , i.af_oracle_ef
                   , i.af_oracle_cia
                   , i.af_ccid
                   , i.af_interface_type
                   , i.af_tipo_uso_convertido
                   , i.af_status_kits
                   , i.af_status_cuentas
                   , i.mmt_mat_trx_tmp_id -- Id de referencia
                   , get_viaje_fnc(i.rh_header_id, i.rl_line_id)
                  );
            EXCEPTION
               WHEN OTHERS
                  THEN
                     fnd_file.put_line (fnd_file.LOG, 'Error al insertar registro en xxinv_pre_material_trx_temp' || SQLCODE || '-' || SQLERRM);
                     ln_Error := 1;
					 lv_Error := 'Error al insertar registro en xxinv_pre_material_trx_temp' || SQLCODE || '-' || SQLERRM;  --HERNAGI. Agregar paramaetro de salida de error.
            END;

         END LOOP; -- fin qty
      END LOOP; -- c_preinsert_mass_additions

      pv_retcode := ln_Error; --HERNAGI. Agregar paramaetro de salida de error.
	  pv_errors  := lv_Error; --HERNAGI. Agregar paramaetro de salida de error.
   ...
   BEGIN
      -- Se busca el viaje
            ...
            AND    wdl.delivery_id IN ( SELECT DISTINCT wda.delivery_id
                                        ...
                                        AND    wdd.source_header_number = p_header_id --order_number
   ...
   ********************************************************************************************/
   ...
   IS
      ln_error      NUMBER := 0; --HERNAGI. Agregar paramaetro de salida de error.
      le_err_msg    VARCHAR2(150) := NULL;
      ln_request_id NUMBER;

      CURSOR cur_pre_mat_trx (p_date IN DATE
	                        , pn_ooh_order_number   IN NUMBER --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
                            , pn_ool_line_id        IN NUMBER --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
	                         )
	  IS
         SELECT *			  
         FROM xxinv_pre_material_trx_temp     
         WHERE 1=1		 
		 AND  (  ( TRUNC(mmt_creation_date) = p_date AND pn_ooh_order_number IS NULL )  --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
		    OR   ( rh_header_id = pn_ooh_order_number AND  rl_line_id = pn_ool_line_id AND pn_ooh_order_number IS NOT NULL ) --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
		      )
      ...
      CURSOR cur_info_escaner (p_viaje IN VARCHAR2
	                         , p_articulo IN VARCHAR2
							 , pn_ooh_order_number   IN NUMBER --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
							 ) IS
         SELECT ses.po_unit_price
              , ses.faa_tag_number
              , ses.faa_serial_number
         FROM   xxfc_sn_escaneo_lineas ses
         WHERE  ses.sn_viaje = p_viaje
         AND    ses.item_number = p_articulo
         AND  ( ( ses.sts_header_id IS NULL AND pn_ooh_order_number IS NULL )  --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
            OR  ( ses.wsh_sts_header_id IS NULL AND pn_ooh_order_number IS NOT NULL )  --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
              )
		 AND  ROWNUM = 1
         FOR UPDATE;

      --CHG0145297 HERNAGI- Inicia Actualiza tabla intermedia. 	
	  ...
	  lv_oracle_cia               xxfc_maestro_de_crs_v.oracle_cia%TYPE;  
      lv_oracle_cr_superior       xxfc_maestro_de_crs_v.oracle_cr_superior%TYPE; 
      lv_oracle_cr_sup_descr      xxfc_centros_responsabilidad.descripcion%TYPE; 
      lv_retek_cr                 xxfc_maestro_de_crs_v.retek_cr%TYPE; 
      lv_oracle_cr_desc           xxfc_maestro_de_crs_v.oracle_cr_desc%TYPE; 
      ...
      --CHG0145297 HERNAGI- Fin Actualiza tabla intermedia. 

   BEGIN

      ln_request_id := FND_GLOBAL.CONC_REQUEST_ID;

      fnd_file.put_line (fnd_file.LOG, 'Se ejecuta el concurrente: ' || ln_request_id || ', para buscar precios de articulos');

      FOR regPMAdd IN cur_pre_mat_trx (pd_creation_date
	                                  ,pn_ooh_order_number --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
									  ,pn_ool_line_id  --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
									   )
	  LOOP
         fnd_file.put_line(fnd_file.LOG, '------------------------------------------------------------------------------------------------');
         fnd_file.put_line(fnd_file.LOG, ' Para este order_number: ' || regPMAdd.rh_header_id || ', se encontro el viaje: ' || regPMAdd.mmt_viaje);
         fnd_file.put_line(fnd_file.LOG, ' Se busca informacion en la tabla de escaneo lineas para el articulo: ' || regPMAdd.msi_segment1_no_articulo);
         fnd_file.put_line(fnd_file.LOG, '------------------------------------------------------------------------------------------------');

         FOR InfScan IN cur_info_escaner (regPMAdd.mmt_viaje
		                                , regPMAdd.msi_segment1_no_articulo
										, pn_ooh_order_number --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
										)
		 LOOP

            fnd_file.put_line(fnd_file.LOG, 'La info para el viaje ' || regPMAdd.mmt_viaje ||
                                            ' y articulo ' || regPMAdd.msi_segment1_no_articulo ||
                                            ' es * Precio: ' || InfScan.po_unit_price ||
                                            ' * Placa: '  || InfScan.faa_tag_number ||
                                            ' * Numero Serie: ' || InfScan.faa_serial_number );
            BEGIN
               UPDATE xxinv_pre_material_trx_temp
               SET    po_unit_price = InfScan.po_unit_price
                    , fa_tag_number = InfScan.faa_tag_number
                    , fa_serial_number = InfScan.faa_serial_number
               WHERE CURRENT OF cur_pre_mat_trx;

               IF pn_ooh_order_number IS NULL
               THEN -- Cuando es ejecutado desde el proceso diario, se debe modificar las columnas  sts_header_id, sts_line_id
                  UPDATE xxfc_sn_escaneo_lineas
                  SET   sts_header_id = regPMAdd.rh_header_id
                      , sts_line_id = regPMAdd.rl_line_id
                  WHERE CURRENT OF cur_info_escaner;
			   ELSE -- Cuando no es ejecutado desde el proceso diario, no se debe modificar las columnas  sts_header_id, sts_line_id
                  UPDATE xxfc_sn_escaneo_lineas
                  SET   wsh_sts_header_id = regPMAdd.rh_header_id
                      , wsh_sts_line_id = regPMAdd.rl_line_id
                  WHERE CURRENT OF cur_info_escaner;
               END IF;
            EXCEPTION
               WHEN OTHERS THEN
                  ln_error   := 1; --HERNAGI. Agregar paramaetro de salida de error.
                  le_err_msg := SUBSTR(SQLERRM, 1, 150);

                  fnd_file.put_line
                     (fnd_file.LOG
                       , 'Error al actualizar los datos del articulo - ' || le_err_msg
                      || ' -> Item_number: '
                      || regPMAdd.msi_segment1_no_articulo
                      || ', Oracle_cr: '
                      || regPMAdd.reql_attribute3_cr
                      || ', Oracle_ef: '
                      || regPMAdd.af_oracle_ef
                      || ', Item_secuencia: '
                      || regPMAdd.mmt_referencia_id
                      || ', Request Id: '
                      || ln_request_id
                     );
            END;
			...
			--CHG0145297 HERNAGI- Inicia Actualiza tabla intermedia. 
            IF pn_ooh_order_number IS NULL -- Cuando se ejecuta desde el proceso diario. 
            THEN -- Cuando es ejecutado desde el proceso diario,			
			   --Reiniciar variables
			   ln_trip_id := NULL;
			   ln_sn_viaje := NULL;
			   ln_purchase_order := NULL;
			   ln_po_unit_price := NULL;
               ...
               BEGIN 
                  ...
                  AND     xsl.sts_header_id      = regPMAdd.rh_header_id
                  AND     xsl.sts_line_id        = regPMAdd.rl_line_id
                  AND     rownum = 1
                  ;        
               EXCEPTION 
                  WHEN NO_DATA_FOUND THEN
                     ln_error   := 1;      
                     le_err_msg :=	SUBSTR('No existe escaneo con viaje para el pedido de movimiento: '||regPMAdd.rh_header_id||' id de linea de pedido de movimiento: '||regPMAdd.rl_line_id,1, 150);			  
                     fnd_file.put_line(fnd_file.LOG, 'No existe escaneo con viaje para el pedido de movimiento: '||regPMAdd.rh_header_id||' id de linea de pedido de movimiento: '||regPMAdd.rl_line_id); 
                  WHEN OTHERS THEN 
                     ln_error   := 1;      
                     le_err_msg :=	SUBSTR('Error al buscar el registro de escaneo con viaje para el pedido de movimiento: '||regPMAdd.rh_header_id||' id de linea de pedido de movimiento: '||regPMAdd.rl_line_id||', others Error: '||SQLERRM,1, 150);			  			   
                     fnd_file.put_line(fnd_file.LOG, 'Error al buscar el registro de escaneo con viaje para el pedido de movimiento: '||regPMAdd.rh_header_id||' id de linea de pedido de movimiento: '||regPMAdd.rl_line_id||', others Error: '||SQLERRM);              
               END;       
               ...
               BEGIN 
                  ...
                  fnd_file.put_line(fnd_file.LOG, 'Buscando registros para actualizar en la tabla intermedia, pedido de compra: '||ln_purchase_order||' articulo: '||regPMAdd.msi_segment1_no_articulo||' precio: '||TO_CHAR(ln_po_unit_price)||' pedido movimiento: '||regPMAdd.rh_header_id||' id de linea de pedido de movimiento: '||regPMAdd.rl_line_id);
                  FOR xxsnrec IN lc_sn_data_details( pv_msi_item_number    => regPMAdd.msi_segment1_no_articulo
                                                   , pn_poh_po_number      => ln_purchase_order
                                                   , pn_po_unit_price      => ln_po_unit_price
                                                    )
                  LOOP
                     -- Buscamos otros atributos de CR 
					 BEGIN 
                        SELECT xmcr.oracle_cia
                             ...
                             , xmcr.oracle_cr_desc   		
                        INTO   lv_oracle_cia
                             , lv_oracle_cr_superior
                             , lv_oracle_cr_sup_descr
                             , lv_retek_cr
                             , lv_oracle_cr_desc							 
						...
						AND   xmcr.estado                = 'A' 
						AND   xmcr.oracle_cr             = regPMAdd.reql_attribute3_cr
						AND   xmcr.oracle_ef             = regPMAdd.af_oracle_ef
                        ;
                     EXCEPTION 
                        WHEN NO_DATA_FOUND THEN
                           ln_error   := 1;      
                           le_err_msg :=	SUBSTR('No se encontro informacion del CR: '||regPMAdd.reql_attribute3_cr||' Estado Financiero: '||regPMAdd.af_oracle_ef, 1, 150);			  
                           fnd_file.put_line(fnd_file.LOG, 'No se encontro informacion del CR: '||regPMAdd.reql_attribute3_cr||' Estado Financiero: '||regPMAdd.af_oracle_ef); 
						   RAISE le_no_exception; 
                        WHEN OTHERS THEN 
                           ln_error   := 1;      
                           le_err_msg :=	SUBSTR('Error al buscar informacion del CR: '||regPMAdd.reql_attribute3_cr||' Estado Financiero: '||regPMAdd.af_oracle_ef||', others Error: '||SQLERRM,1, 150);			  			   
                           fnd_file.put_line(fnd_file.LOG, 'Error al buscar informacion del CR: '||regPMAdd.reql_attribute3_cr||' Estado Financiero: '||regPMAdd.af_oracle_ef||', others Error: '||SQLERRM);    
                           RAISE le_no_exception;  						   
                     END; 
					 
					 --Obtenemos el id del pedido de movimiento
                     -- Buscamos otros atributos de CR 
					 BEGIN 
                        ...
                        WHERE 1=1
                        AND   oeh.order_number = regPMAdd.rh_header_id
                        ;
                     EXCEPTION 
                        WHEN NO_DATA_FOUND THEN
                           ln_error   := 1;      
                           le_err_msg :=	SUBSTR('No se encontro informacion del pedido de movimiento: '||regPMAdd.rh_header_id, 1, 150);			  
                           fnd_file.put_line(fnd_file.LOG, 'No se encontro informacion del pedido de movimiento: '||regPMAdd.rh_header_id); 
						   RAISE le_no_exception; 
                        WHEN OTHERS THEN 
                           ln_error   := 1;      
                           le_err_msg :=	SUBSTR('Error al buscar pedido de movimiento: '||regPMAdd.rh_header_id||', others Error: '||SQLERRM,1, 150);			  			   
                           fnd_file.put_line(fnd_file.LOG, 'Error al buscar pedido de movimiento: '||regPMAdd.rh_header_id||', others Error: '||SQLERRM);    
                           RAISE le_no_exception;  						   
                     END; 				 
					 
                     -- Actualizar registro. 
                     XXFA_SN_DATA_DETAILS_PKG.update_row( p_data_detail_id             => xxsnrec.data_detail_id
                                                        , p_prl_requisition_line_id    => regPMAdd.rl_attribute1_req_line_id
                                                        , p_prl_oracle_cia             => lv_oracle_cia
                                                        , p_prl_oracle_ef              => regPMAdd.af_oracle_ef
                                                        , p_prl_retek_distrito         => lv_retek_cr
                                                        , p_prl_oracle_cr              => regPMAdd.reql_attribute3_cr
                                                        , p_prl_oracle_cr_descr        => lv_oracle_cr_desc
                                                        , p_prl_oracle_cr_superior     => lv_oracle_cr_superior
                                                        , p_prl_oracle_cr_sup_descr    => lv_oracle_cr_sup_descr
                                                        , p_ooh_header_id              => ln_order_header_id
                                                        , p_ooh_order_number           => regPMAdd.rh_header_id
                                                        , p_ool_line_id                => regPMAdd.rl_line_id
                                                        , p_wst_trip_id                => ln_trip_id
                                                        , p_wst_trip_name              => ln_sn_viaje
                                                        , p_mmt_transaction_id         => regPMAdd.mmt_transaction_id
                                                        , p_mmt_creation_date          => regPMAdd.mmt_creation_date
                                                        , x_errors                     => lv_x_errors
                                                        , x_retcode                    => ln_x_retcode
                                                        );
                     IF ln_x_retcode = 0 
                     THEN 
                        fnd_file.put_line(fnd_file.LOG, 'Se actualizo el registro de xxfa_sn_data_details, pedido de compra: '||ln_purchase_order||' articulo: '||regPMAdd.msi_segment1_no_articulo||' precio: '||TO_CHAR(ln_po_unit_price)||' id data detail: '||xxsnrec.data_detail_id||' pedido movimiento/viaje '||regPMAdd.rh_header_id||'/'||ln_sn_viaje);
                     ...
                     ELSE 
                        ln_error   := 1;		
                        le_err_msg :=	SUBSTR( 'Error al actualizar el registro de xxfa_sn_data_details, pedido de compra: '||ln_purchase_order||' articulo: '||regPMAdd.msi_segment1_no_articulo||' precio: '||TO_CHAR(ln_po_unit_price)||' id data detail: '||xxsnrec.data_detail_id||' pedido movimiento/viaje '||regPMAdd.rh_header_id||'/'||ln_sn_viaje||' Error: '||lv_x_errors,1, 150);			  							
                        fnd_file.put_line(fnd_file.LOG, 'Error al actualizar el registro de xxfa_sn_data_details, pedido de compra: '||ln_purchase_order||' articulo: '||regPMAdd.msi_segment1_no_articulo||' precio: '||TO_CHAR(ln_po_unit_price)||' id data detail: '||xxsnrec.data_detail_id||' pedido movimiento/viaje '||regPMAdd.rh_header_id||'/'||ln_sn_viaje||' Error: '||lv_x_errors);  
                        RAISE le_no_exception; 
                     END IF;         
                  END LOOP;       
               EXCEPTION
                  ...
                  WHEN OTHERS THEN
				     ln_error   := 1;
					 le_err_msg :=	SUBSTR(  'Error actualizando tabla intermedia a partir de las referencias en xxinv_pre_material_trx_temp y xxfc_sn_escaneo_lineas para el pedido de movimiento, others: '||SQLERRM,1, 150);			  				
         ...
         END LOOP;
      END LOOP; -- fin procesa_info_articulo_prc
      pv_retcode   := ln_error; --HERNAGI. Agregar paramaetro de salida de error.
	  pv_errors    := le_err_msg; --HERNAGI. Agregar paramaetro de salida de error.
   EXCEPTION
      WHEN OTHERS THEN
         fnd_file.put_line(fnd_file.LOG, 'Ocurrio un error al ejecutar el prodecimiento procesa_info_articulo_prc. ' || SQLERRM);
		 pv_retcode   := 2; --HERNAGI. Agregar paramaetro de salida de error.
		 pv_errors    := 'Ocurrio un error al ejecutar el prodecimiento procesa_info_articulo_prc. ' || SQLERRM; --HERNAGI. Agregar paramaetro de salida de error.
   ...
   ********************************************************************************************/
   ...
   BEGIN
      -- Se depura la informacion inicial.
      DELETE FROM xxinv_material_trx_temp
      WHERE  (  ( rh_header_id = pn_ooh_order_number AND  rl_line_id = pn_ool_line_id ) OR pn_ooh_order_number IS NULL ) --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
	  ;

      IF pn_ooh_order_number IS NULL
	  THEN --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
         -- Se carga de nuevo la informacion agrupada por precio
         INSERT INTO xxinv_material_trx_temp (
             reqh_segment1_no_requisicion
           , reqh_requisition_header_id
           , reql_attribute2_crsup
           , reql_attribute3_cr
         ...
         SELECT reqh_segment1_no_requisicion
              ...
              , mmt_transaction_date
              , mmt_distribution_account_id
              , NVL(po_unit_price, mmt_actual_cost)
              , mmt_source_code
              , mmt_attribute1
              , mmt_attribute2
              , mmt_attribute3
              ...
              , po_unit_price
              , mmt_referencia_id
              , xxinv_ref_lines_s.NEXTVAL
              , mmt_actual_cost
         FROM ( SELECT reqh_segment1_no_requisicion
                     , reqh_requisition_header_id
                     , reql_attribute2_crsup
                     , reql_attribute3_cr
                     , msi_segment1_no_articulo
                     ...
                     , mmt_created_by
                     , mmt_inventory_item_id
                     , SUM(mmt_transaction_quantity) mmt_transaction_quantity
                     , mmt_transaction_date
                     , mmt_distribution_account_id
                     , mmt_actual_cost
                     , mmt_source_code
                     ...
                     , mmt_referencia_id
                FROM  xxinv_pre_material_trx_temp
                WHERE 1=1--trunc(mmt_creation_date) = '13-NOV-2024'
                GROUP BY reqh_segment1_no_requisicion
                       , reqh_requisition_header_id
                       , reql_attribute2_crsup
                       , reql_attribute3_cr
                       , msi_segment1_no_articulo
      ...
      --Una vez finalizado el proceso de actualizacion del precio de compra se depura la tabla de paso.
      DELETE FROM XXINV_PRE_MATERIAL_TRX_TEMP
      WHERE  (  ( rh_header_id = pn_ooh_order_number AND  rl_line_id = pn_ool_line_id ) OR pn_ooh_order_number IS NULL ) --HERNAGI. CHG0116809 Procesar por registro de linea de pedido de movimiento.
	  ;

	  pv_retcode := 0; --HERNAGI. Agregar paramaetro de salida de error.

   EXCEPTION
      WHEN OTHERS THEN
         fnd_file.put_line(fnd_file.LOG, 'Ocurrio un error al ejecutar el prodecimiento reagrupa_info_prc. ' || SQLERRM);
		 pv_retcode := 1; --HERNAGI. Agregar paramaetro de salida de error.
		 pv_errors  := 'Ocurrio un error al ejecutar el prodecimiento reagrupa_info_prc. ' || SQLERRM; --HERNAGI. Agregar paramaetro de salida de error.
   END;
   ...
   ********************************************************************************************/
   ...
   IS
   BEGIN
      -- Se inserta en la tabla temporal a partir de una linea de pedido de movimiento.
      INSERT INTO xxinv_material_trx_temp
      ( reqh_segment1_no_requisicion
      , reqh_requisition_header_id
      , reql_attribute2_crsup
      ...
      FROM  mtl_material_transactions mmt
          ...
          , oe_order_headers_all rh
          , oe_order_lines_all rl
          , po_requisition_headers_all reqs
      ...
      --AND    mta.reference_account           = mmt.distribution_account_id
      --AND    mta.accounting_line_type        = 36
      AND    rh.order_number                 = pn_ooh_order_number
      AND    rl.line_id                      = pn_ool_line_id
      ;
